...
const updateConversation = async (index) => {
  console.log("Update conversation failed")
}

const deleteConversation = async (index) => {
  deletingConversationIndex.value = index
  const { data, error } = await useAuthFetch(`/copilot/chat/session/${conversations.value[index].session_id}`, {
    method: 'DELETE'
  })
...
const exportConversation = async (index) => {
  ...
  // data.messages = []
  // let messages = await loadMessage(conversation.session_id)
...
const signOut = async () => {
  const { data, error } = await useAuthFetch('/account/logout/', {
    method: 'POST'
  })
  if (!error.value) {
    await logout()
      ...
      <v-list>
        ...
        >
          <v-list-item
              ...
              rounded="xl"
              v-if="editingConversation && editingConversation.session_id === conversation.session_id"
          >
            <v-text-field
                v-model="editingConversation.session_id"
          ...
          >
            <v-list-item
                rounded="xl"
                active-color="primary"
                :to="conversation.session_id ? `/${conversation.session_id}` : '/'"
                v-bind="props"
            >
              <v-list-item-title>{{ (conversation.session_name && conversation.session_name !== '') ? conversation.session_name : $t('defaultConversationTitle') }}</v-list-item-title>
              <template v-slot:append>
                <div
                    v-show="isHovering && conversation.session_id"
                >
                  <!-- <v-btn
                      icon="edit"
                      size="small"
